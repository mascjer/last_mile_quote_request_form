...
def check_c_codes(ccode):
    if ccode is None:
        ccode = ''
    c_string = "SELECT DISTINCT rtrim(ltrim(CUSTOMERCODE)) CUSTOMER_PARTY_CODE FROM NAST_TRUCKLOAD_DOMAIN.BROKER.DIM_CUSTOMER WHERE rtrim(ltrim(CUSTOMERCODE)) = '" + ccode + "'"
...
def check_emp_codes(empcode):
    if empcode is None:
        empcode = ''
    emp_string = "SELECT SEVEN_LETTER FROM ENTERPRISE_REFERENCE_DOMAIN.BASE.REF_WORKER WHERE SEVEN_LETTER ='" + empcode + "'"
